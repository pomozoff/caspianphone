...
- (void)setupView
{
	...
	// Table View modified by  on 17 March
    CGRect tableFrame = CGRectMake(0.0f, 70.0f, self.view.frame.size.width, self.view.frame.size.height - kInputHeight);
    ...
    NSString *title = NSLocalizedString(@"SMS",);
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (type == AMBubbleCellTimestamp) {
		...
		NSString* username=nil; // Modified by  on 18 March to remove warning
...
- (void)sendPressed:(id)sender
{
    ...
    NSString *str_messageViewField = [NSString stringWithFormat:@"%@",self.textView.text];  // Text field content
    NSString *str_finalPhoneNumber = [[NSUserDefaults standardUserDefaults] objectForKey:@"finalPhoneNumber"];  // Phone number
    NSString *str_mblNumber = [[NSUserDefaults standardUserDefaults] objectForKey:caspianPhoneNumber];   // Username
    ...
    NSLog(@"Text field says:%@",str_messageViewField);
    NSLog(@"Phone Number says:%@",str_finalPhoneNumber);
   // NSString *urlForHttpGet = [NSString stringWithFormat:@"http://india.msg91.com/sendhttp.php?user=khanna&password=253912&mobiles=+%@&message=%@&sender=GOBZAR",str_finalPhoneNumber,str_messageViewField];
